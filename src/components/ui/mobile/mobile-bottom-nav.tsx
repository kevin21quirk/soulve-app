...
const MobileBottomNav = ({ activeTab, onTabChange }: MobileBottomNavProps) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 safe-area-bottom">
      <TooltipProvider>
        <div className="grid grid-cols-7 gap-0.5 py-1 px-1">
          {tabs.map((tab) => {
              ...
              <Tooltip key={tab.value}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-1.5 min-h-[56px] rounded-md transition-all duration-200 ${
                      isActive 
                        ? 'bg-gradient-to-r from-[#0ce4af] to-[#18a5fe] text-white' 
                        : 'bg-transparent text-gray-600 hover:bg-gray-100'
                    }`}
                  >
                    <IconComponent className="h-4 w-4 mb-0.5 flex-shrink-0" />
                    <span className="text-[10px] leading-none text-center truncate w-full px-0.5 font-medium">
                      {tab.label}
                    </span>
                  </button>
                </TooltipTrigger>

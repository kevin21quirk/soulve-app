...
const CompactRecommendationCard = ({ recommendation, onAction }: CompactRecommendationCardProps) => {
  const getTypeDisplay = () => {
    switch (recommendation.type) {
      case "connection":
        return { label: "Connect", color: "bg-blue-100 text-blue-800" };
      case "help_opportunity":
        return { label: "Help", color: "bg-orange-100 text-orange-800" };
      case "skill_match":
        return { label: "Tutor", color: "bg-purple-100 text-purple-800" };
      case "post":
        return { label: "Join", color: "bg-green-100 text-green-800" };
      default:
    ...
    <div className="p-3 border border-purple-200 rounded-lg hover:shadow-md transition-all duration-200 bg-white hover:bg-purple-50 group">
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          <Badge className={`text-xs px-1.5 py-0.5 ${typeDisplay.color}`}>
            {typeDisplay.label}
          </Badge>
        </div>
      ...
      <div className="mb-2">
        <h4 className="font-medium text-sm text-gray-900 line-clamp-1">{recommendation.title}</h4>
      ...
      {recommendation.type === "connection" && (
        <div className="flex items-center space-x-2 mb-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-gray-900 truncate">{recommendation.data.user}</p>
            <p className="text-xs text-gray-500 truncate">{recommendation.data.location}</p>
          </div>
        </div>
      )}
      ...
      {recommendation.type === "help_opportunity" && (
        <div className="mb-2">
          <p className="text-xs text-gray-600">{recommendation.data.location}</p>
      ...
      {recommendation.type === "skill_match" && (
        <div className="mb-2">
          <p className="text-xs text-gray-600">{recommendation.data.subject}</p>
          <p className="text-xs text-gray-500">{recommendation.data.schedule}</p>
        </div>
      )}
      ...
      {recommendation.type === "post" && (
        <div className="mb-2">
          <p className="text-xs text-gray-600">{recommendation.data.organizer}</p>
          <p className="text-xs text-gray-500">{recommendation.data.date}</p>
        </div>
      )}
      ...
      <Button
        ...
        size="sm"
        className="w-full h-7 text-xs group-hover:bg-purple-600 group-hover:text-white transition-colors"
        variant="outline"
      >

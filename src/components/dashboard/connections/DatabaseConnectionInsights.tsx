...
interface DatabaseConnectionInsightsProps {
...
const DatabaseConnectionInsights = ({ connectedPeople, suggestedConnections }: DatabaseConnectionInsightsProps) => {
  // Calculate insights from connections
  const topSkills = connectedPeople
    .flatMap(person => person.partner_profile?.skills || [])
    .reduce((acc, skill) => {
  ...
  const topLocations = connectedPeople
    .map(person => person.partner_profile?.location || 'Unknown')
...
export default DatabaseConnectionInsights;

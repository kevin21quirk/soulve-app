-- Fix critical security vulnerability in messages table (corrected version)
...
-- Policy for updating messages: Only participants can update (for read status mainly)
CREATE POLICY "Users can only update messages they have access to" 
ON public.messages 
FOR UPDATE 
USING (
  public.can_access_message(sender_id, recipient_id)
